...
static uint64_t receive(TPCANHandle channel, ipc_server_t server);
static uint64_t receive_fd(TPCANHandle channel, ipc_server_t server);
...
int main(int argc, char *argv[])
{
    ...
    *p++ = '\0';
    ...
    if (sscanf(p, "%" SCNi64 "%n", &intarg, &n) != 1 || p[n] != '\0') {
        fprintf(stderr, "%s: illegal argument for <port> (`%s')\n", basename(argv[0]), p);
        return 1;
    }
    ...
    if (!(op_mode & PCAN_MESSAGE_FD))
        ...
        server = ipc_server_start(port, IPC_SOCK_TCP, sizeof(struct canfd_frame), transmit_fd, (void*)&channel, logging);
    if (server == NULL) {
        fprintf(stdout, "FAILED!\n");
...
static uint64_t receive(TPCANHandle channel, ipc_server_t server)
{
    ...
    while (running) {
        if ((status = CAN_Read(channel, &message, &timestamp)) == PCAN_ERROR_OK) {
            ...
            (void)ipc_server_send(server, (const void*)&packet, sizeof(packet));
        }
