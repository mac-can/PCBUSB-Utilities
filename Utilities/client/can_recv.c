...
int main() {
    ...
    while (running) {
        ...
        if ((n = ipc_client_recv(fildes, (void*)&msg, sizeof(msg), IPC_WAIT_FOREVER)) < 0) {
            if (errno != ENODATA)
                perror("+++ error");
            if (errno == EBADMSG)
                break;
        ...
        if (n != (ssize_t)sizeof(msg)) {
            fprintf(stderr, "+++ error: message with wrong MTU received\n");
            break;
        }
        ...
        printf(" [%u]", msg.len);
        for (uint8_t i = 0; (i < msg.len) && (i < CAN_MAX_DLEN); i++) {
    ...
    printf("Connection closed: %u CAN frames received\n", frames);
    return EXIT_SUCCESS;
}
